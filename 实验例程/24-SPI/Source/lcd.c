...
char* experiment_description[38] = {"使用FSMC驱动LCD液晶屏，并在屏幕上显示文字。",
                                    ...
                                    "通过IIC总线读取光照度传感器寄存器数据,串口每秒打印一次数据。",                
...
void lcd_init(unsigned char name)
{
  ...
  LCDDrawFnt16(4,32,4,320,"实验描述：",0x4596,-1);
  LCDDrawFnt16(4+32,52,4,320,experiment_description[name-1],0x0000,-1);
  LCDDrawFnt16(4,32+20*5,4,320,"实验现象：",0x4596,-1);
  LCD_Clear(0, 240-30, 319, 240,0x4596);
  LCDDrawFnt24(76, 213, "嵌入式接口技术", 0xffff, 0x4596);
}
